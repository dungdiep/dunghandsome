...
if (isset($_POST['address']))
    ...
    if (isset($_POST['earn']))
        $earn = trim($_POST["earn"]);
    else $earn  = "";

    //Kiểm tra điều kiện bắt buộc đối với các field không được bỏ trống
    if (
        $id == "" || $product_id == ""|| $address=="" || $quantity ==
        "" || $earn == ""
    ) {
        echo '<script>';
        ...
        $sql = "SELECT * from transaction where product_id='$product_id' ";

        $user = mysqli_query($conn, $sql);

        if (mysqli_num_rows($user)  > 0) {
            echo '<script language="javascript">';
            echo 'alert("Sản phẩm đã có trong đơn hàng.")';
            echo '</script>';
        } else {
            ...
            VALUES (NULL, '$id','$product_id','$address','$quantity', '$earn',Null)";
        ...
        <form method="post" action="add_order.php">
            ...
            <div class="row">
                <div class="col-md-12 cart">
                    <div class="title">
                        <div class="row">
                            <div class="col text-muted">
                                <h4><b style="color: green;"> <i class="far fa-address-card"></i>Lịch sử giao dịch</b>
                                </h4>
                            </div>
                    ...
                    <div class="row border-top border-bottom p-2">
                        ...
                        <div class="row main align-items-center">
                            ...
                            <input type="text" name="quantity" value="<?php echo $row['quantity'] ?> "
                                style="display: none;">
                            <input type="text" name="price" value="<?php echo $row['earn'] ?> " style="display: none;">
                            ...
                            <div class="col">
                                ...
                                <form action="updateQuantity.php" method="GET">
                                    <input name="cartID" type="text" value="<?php echo $row['order_id']; ?>"
                                        style="display: none;">
                                    <div class="quantity">
                                        <input disabled="disabled" name="quantity" type="number"
                            ...
                            <div class="col" style="color: red;">
                                <?php echo adddotstring($row['price']); ?> VND
                            </div>
                    ...
                    <br>
                    <div class="col-2">
                        <div class="row text-muted"><button class="btn btn-warning bg-warning"
                                style=" border-radius: 10px;" type="submit" name="save">
                                Lưu đơn hàng</button></div>
                    </div>
                    <div style="float: right; padding: 5px;">
                        <?php
                        $conn = mysqli_connect('localhost', 'root', '', 'onlineshop');
                        $earn = mysqli_query($conn, "SELECT SUM(earn) FROM transaction");
                        $sumorder = mysqli_query($conn, "SELECT count(1) as summary FROM transaction");
                        ?>
                        <p> Tổng tiền:
                            <?php
                            $i = 0;
                            while ($row = mysqli_fetch_array($earn)) {
                            ?>
